class GruposController < ApplicationController
  ...
  before_filter :correct_estatuto, :only => [:edit, :update, :new, :destroy]
  before_filter :ultimoAdmin, :only => [:destroy]

  def ultimoAdmin
    @grupo = Grupo.find(params[:id])
    redirect_to grupos_path, notice: "Este grupo possui os ultimos administradores!" unless Utilizador.find_all_by_estatuto("Administrador").count != @grupo.utilizadors.find_all_by_estatuto("Administrador").count
  end
  ...
  def correct_estatuto
    redirect_to grupos_path, notice: "Precisa de ser administrador!" unless estatuto_admin?
  end
end
